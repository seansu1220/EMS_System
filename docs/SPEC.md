...
> 版本：v1.18.2　建立日期：2026-07-22　最後更新：2026-08-07
...
| 狀態 | 顯示 | 意思 |
| --- | --- | --- |
| `pending` | 待處理 | 已送出，等救護科電腦執行 |
| `running` | 處理中 | 電腦正在跑這一筆 |
| `unlocked` | 已解鎖 | 完成，附案件日期、出勤車輛與依據 |
| `noAction` | 本來就沒鎖 | 那張紀錄表已是未結案，不需要動作 |
| `failed` | 需人工處理 | 程式判斷不出來，附原因 |
...
- `unlockRequests`（v1.17 起）：
  ...
  - 更新（回寫結果）：僅 `isStaff()`，且不可竄改 `temsis`／`reason`／`requestedBy`／`requestedAt`。
  - 刪除：僅管理員。
...
- 規則測試腳本置於 `scripts/rules.test.mjs`，執行 `npm run test:rules`（v1.17 起 **49/49 通過**）。
