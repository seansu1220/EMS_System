...
> 版本：v1.10.1　建立日期：2026-07-22　最後更新：2026-07-26
...
| 查詢期間 | `#_txtFromDate`、`#_txtToDate`（My97DatePicker，格式於執行時偵測） |
| 救護狀態 | `#_selSTATUS`，已結案＝`0` |
| 院前預警 | `#_selPreHOSPWarning`，到院前傳送預警＝`1` |
| 分隊 | `#_selDeptnoCar`（58 個分隊，含代碼） |
| 查詢 / 匯出EXCEL | `#_btnQuery` / `#_btnExcel`（**圖片按鈕，非標準表單元件**） |
| 進階搜尋展開 | `triggleTable()`、`triggleTable2()`（同頁展開，不是彈出視窗） |

導航注意：**上方 `header` 那排報表連結要進入報表系統後才存在**，登入直後只有「首頁」，
不可用來導航；一律走左側選單，並保留「直接載入內容框網址」作為備援。
...
| `scrape.mjs` | 設定條件、查詢、攔截並儲存兩份匯出檔 |
| `workbook.mjs` | 匯出檔解析（自動定位標題列）與只含結構的診斷資訊 |
| `aggregate.mjs` | 分隊計數、比較、合計（**純函式**） |
...
### 4.7 統計規則
- 以匯出檔的「分隊」欄位計數（欄名候選見 `SQUAD_COLUMN_CANDIDATES`，找不到時列出實際欄名報錯）。
...
- ✅ 查詢、兩次匯出、分隊彙總、報表輸出（單元＋整合測試 14 項通過）
- ⏳ 尚未對真實系統實跑驗證（日期格式、匯出檔欄名以實跑結果為準）
