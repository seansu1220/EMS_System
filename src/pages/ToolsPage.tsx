...
const LOCAL_TOOLS: readonly {
  ...
  },
  {
    ...
    purpose:
      '把各分隊在「解鎖工單」頁提出的申請處理完，結果自動回到網頁上，不用再打電話回覆。有兩種跑法：一次性、或整天開著自動處理。',
    shortcut: '捷徑 \\ 線上解鎖工單.bat（一次性）／線上解鎖工單-自動監看.bat（整天開著）',
    steps: [
      '一次性：雙擊「線上解鎖工單.bat」，它會先告訴你有幾筆待處理；一筆都沒有的話連瀏覽器都不會開。',
      '自動監看：雙擊「線上解鎖工單-自動監看.bat」並讓視窗開著。有人送出申請後大約 20 秒內就會被處理，你不用做任何事。',
      '兩種都是打一次驗證碼。監看版會每 5 分鐘輕輕碰一下系統，讓登入不要逾時；真的被踢掉時會停下來請你再打一次。',
      '每解完一筆就立刻回寫一筆，申請人在網頁上看得到。判斷不明確的一律略過，不會用猜的。',
    ],
