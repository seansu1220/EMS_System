...
const LOCAL_TOOLS: readonly {
  ...
  },
  {
    ...
    steps: [
      ...
      '另外會產出「到院前未預警清冊」：送醫卻沒有到院前預警的每一件都列出來，並標明有沒有被扣除。分隊來問「我是哪幾件」時直接開這份，不用重跑。',
      '增減試算表的扣除會逐列對帳：拿表上的 TEMSIS 去找那件案子，確認它真的沒有到院前預警才扣。查不到、或那件本來就有預警的一律不扣，跑完會逐件列出來讓你去問分隊。',
    ],
    note: '對帳結果寫在未預警清冊的第三個分頁。這份含完整 TEMSIS，放在 tools\\ems-report\\out\\internal\\，不要整份發給分隊。',
  },
  {
  ...
  {
    title: '二級以上因交通事故救護案件（來文格式一覽表）',
    purpose:
      '查出上個月「已結案、因交通事故受傷、到院後檢傷第 1 或第 2 級」的案件，直接填成來文的一覽表格式。認定只看到院後檢傷分級，是不是危急個案不管。',
    shortcut: '捷徑 \\ 二級以上因交通事故救護案件 \\ 產出來文格式.bat',
    steps: [
      '雙擊上面那個檔案，程式開啟瀏覽器停在登入頁，請你本人輸入驗證碼後按登入。',
      '接著全自動：檢傷第 1 級查一次、第 2 級查一次（系統的下拉只能單選），各匯出一份再合併成一張表。',
      '日期寫成民國格式（115/08/01），依發生日期由早到晚排，編號重新給；醫護人員檢傷分級取「到院後檢傷分級」，五級分類取「到院前檢傷分級」。',
      '產出在 tools\\ems-report\\out\\internal\\，檔名是「年月-二級以上因交通事故救護案件.xlsx」（年月排在最前面，資料夾才照時間排）。',
    ],
    note: '這份含姓名與身分證字號，所以放在 internal 資料夾（不能整包發給分隊）。姓名「不詳」、沒有身分證字號的案件會照實列出並在畫面上告訴你有幾件。查詢頁上有兩組長得一模一樣的欄位，程式匯出後會逐列回頭核對條件，對不上就換另一個欄位重來，都不對就停手不產出。',
  },
  {
    title: '解鎖救護紀錄表',
    purpose:
