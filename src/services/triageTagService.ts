 ...
 * `H00T010`、`H00T011`。為了讓兩個人同時按也不會拿到同一批號碼，
 * 「讀下一號 → 往後推 → 寫領取紀錄」包在同一個 Firestore transaction 裡，
 * 安全規則再檢查一次兩份文件對得起來（見 `firebase/firestore.rules` 的 triageTag 段落）。
 *
...
import { formatTriageTagNumber, remainingTriageTags } from '../lib/triageTagNumber';
 ...
 */
export function validateTriageTagCount(count: number, nextSerial: number): string | null {
...
/**
 * 領取一批傷票號碼（接續上一個人的號碼往下發）。
 *
 * @returns 這次拿到的起始流水號與張數
 * @throws 張數不合法、號碼不夠、或寫入失敗時
 */
export async function allocateTriageTags(
  ...
  try {
    return await runTransaction(db, async (transaction) => {
      ...
      const problem = validateTriageTagCount(count, startSerial);
      if (problem) throw new Error(problem);
      transaction.set(counterRef, { nextSerial: startSerial + count, updatedAt: serverTimestamp() });
      // 文件 ID＝起始流水號：天生不重複，安全規則也靠它把兩份文件對起來。
