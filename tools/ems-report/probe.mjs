 ...
 */
export async function runInteractiveProbe(page) {
  ...
  for (;;) {
    ...
    const label = frames.find((frame) => frame.title)?.title ?? 'page';
    const fileName = `${toSafeLabel(label, sequence)}.json`;
    const filePath = path.join(PATHS.probeDir, fileName);
    await fs.writeFile(filePath, JSON.stringify({ capturedAt: new Date().toISOString(), frames }, null, 2), 'utf8');

    log.ok(`已存出：${path.relative(process.cwd(), filePath)}（${frames.length} 個 frame）`);
    sequence += 1;
  }
  log.ok('探測模式結束');
}
