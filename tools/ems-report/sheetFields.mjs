...
/**
 * 列出文字中看起來像「欄位標籤」的字串，去重。
 *
 * 只在需要的欄位抓不到時用來排查——**標籤是表單的結構，不是個人資料**，
 * 與探測模式只記錄欄位名稱的作法一致。
 *
 * 只認 `受理時間：` 這種**有冒號**的樣式。冒號幾乎不會出現在值裡面，
 * 因此抓到的確實都是欄位名稱，不會把個案內容帶出來。
 *
 * 沒有冒號的排版就抓不到——這是刻意的取捨：試過「中文詞後面緊跟數字」的備援樣式，
 * 但它分不出「標籤」與「值」（`出勤車輛 桃園91` 抓到的是「桃園」而不是「出勤車輛」），
 * 既不可靠又可能帶出內容。這種情況改為請使用者直接看一張紀錄表告知欄位名稱。
 *
 * @param {string} text
 * @param {number} [limit]
 * @returns {string[]}
