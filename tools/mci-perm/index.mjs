...
import { CLEAR_ALL, CLEAR_STATIONS, GRANT_UNITS, PATHS, SITE } from './config.mjs';
...
const COMMANDS = ['grant', 'grant-units', 'clear-all', 'clear-stations', 'probe'];
...
export function parseArgs(args) {
  ...
  return {
    command: /** @type {'grant'|'grant-units'|'clear-all'|'clear-stations'|'probe'} */ (command),
...
/**
 * clear-stations：只把**各分隊**的 MCI 權限取消，科室與大隊維持現狀。
 *
 * 使用者 2026-08-29 的要求：「把各分隊目前有權限的人都刪掉，只有 OO 分隊要，
 * 如果是科室或者大隊就維持現狀」。
 *
 * 與 `clear-all` 的差別只在挑單位那一步：那支是「除了保留清單以外全部都清」，
 * 這支是「名稱裡有『分隊』的才清」。其餘（掃描、續跑、回讀查證）完全共用。
 *
 * @returns {SweepPlan}
 */
function clearStationsPlan() {
  const keywords = CLEAR_STATIONS.keywords();
  const keepUnits = CLEAR_STATIONS.keepUnits();
  return {
    label: '取消分隊',
    title: '取消分隊權限：把各分隊裡有「MCI 大量傷病患救護管理系統」權限的人取消掉',
    intro: [
      `只處理名稱含「${keywords.join('」「')}」的單位（在 .env 用 MCI_CLEAR_UNIT_KEYWORDS 可以改）`,
      '科室、大隊、中心一律維持現狀，一個人都不會被動到。',
      `另外保留不動：${keepUnits.join('、')}（MCI_KEEP_UNITS）`,
    ],
    chooseTargets(unitOptions) {
      const { targets, skipped, kept } = splitUnitsByKeyword(unitOptions, keywords, keepUnits);
      // ⚠ 一個分隊都沒認出來＝系統換了單位的寫法，或關鍵字設錯。
      //   這時候「沒事做」是假象，繼續跑只會什麼都沒清掉還以為做完了。
      if (targets.length === 0) {
        throw new Error(
          `單位下拉裡找不到任何名稱含「${keywords.join('」「')}」的單位，這次停手不做。
` +
            `　下拉裡實際有這些單位：${unitOptions.map((option) => option.text).filter(Boolean).join('、')}
` +
            '　請照系統裡的寫法改 .env 的 MCI_CLEAR_UNIT_KEYWORDS。',
        );
      }
      log.ok(`要處理的分隊共 ${targets.length} 個：${targets.map((option) => option.text).join('、')}`);
      if (kept.length > 0) {
        log.ok(`保留不動：${kept.map((option) => option.text).join('、')}`);
      }
      // 沒被挑到的單位一定要印出來讓人看過一遍：萬一有分隊的名稱不叫「分隊」
      // （例如「XX救護隊」），它會安靜地留在這裡，只有承辦人看得出來不對。
      if (skipped.length > 0) {
        log.info(`不在處理範圍的單位共 ${skipped.length} 個（科室、大隊等）：`);
        log.info(`　${skipped.map((option) => option.text).join('、')}`);
        log.info('　請看一眼有沒有「其實是分隊但名字不叫分隊」的漏網之魚。');
      }
      return targets;
    },
    handleAll: revokeAll,
    ...
    confirmLines: (count) => [
      `即將把 ${count} 位的「MCI 大量傷病患救護管理系統」勾選取消掉，這會真的寫進系統。`,
      `範圍只有名稱含「${keywords.join('」「')}」的單位；科室與大隊維持現狀。`,
      '本來就沒有這個權限的人會直接跳過，一顆確定都不會按。',
    ],
    dryRunNote: '試跑模式：只會看每個人現在的狀態，不會取消任何權限',
  };
...
async function main() {
  ...
  else if (options.command === 'clear-stations') await runUnitSweep(options, clearStationsPlan());
